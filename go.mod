module github.com/razzie/riddle-solver

go 1.16

require (
	gioui.org v0.0.0-20210909142423-e92ca233f5b1
	gioui.org/x v0.0.0-20210816192830-9ea938c228a0
	github.com/antonmedv/expr v1.3.1
	github.com/gdamore/tcell v1.1.2
	github.com/razzie/dlgs v0.0.0-20210910202245-f927eda844b9
	github.com/razzie/razgio v0.0.0-20210910000152-7dfc0f2528fb
	github.com/rivo/tview v0.0.0-20190829161255-f8bc69b90341
)
